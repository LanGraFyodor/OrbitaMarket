...
| product persistence | готовый Blob хранится в IndexedDB; ключей изображения в `localStorage` нет |
| уведомления | результат оплаты появляется в истории и SSE-колокольчике |
| API negative paths | неверный UUID Orders и отсутствие `X-User-Id` Notifications дают единый JSON 400, не 500 |
